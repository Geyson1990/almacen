...
import { FormularioRequestPost } from '../../models/Formularios/Formulario006_17/FormularioRequestPost';

@Injectable({
  providedIn: 'root'
})
export class Formulario00617Service {

  urlApi: string = '';
  urlApiPdf: string = '';

  constructor(private httpClient: HttpClient) {
    this.urlApi = `${environment.baseUrlAPI}${environment.endPoint.formulario.formulario6_17}/`;
  ...
  readPostFie(id:number): Observable<any> {
    ...
    return this.httpClient.get(this.urlApiPdf + `formulario006_17?id=${id}`, htmlOptions);
  }
