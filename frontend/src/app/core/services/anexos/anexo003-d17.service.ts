...
export class Anexo003D17Service {
  urlApi: string = '';
  urlAnexoRead: string = '';
  urlApiPdf: string = '';

  constructor(private httpClient: HttpClient) {
    this.urlApi = `${environment.baseUrlAPI}${environment.endPoint.anexos.anexo3_d17}/`;
  ...
  readPostFie(id: number): Observable<any> {
    ...
    return this.httpClient.get(this.urlApiPdf + `anexo003_d17?id=${id}`, htmlOptions);
  }
