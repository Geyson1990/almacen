...
export class Anexo001D27NTService {
  urlApi: string = '';
  urlAnexoRead: string = '';
  urlApiPdf: string = '';

  constructor(private httpClient: HttpClient) {
    this.urlApi = `${environment.baseUrlAPI}${environment.endPoint.anexos.anexo1_d27NT}/`;
  ...
  readPostFie(id: number): Observable<any> {
    ...
    return this.httpClient.get(this.urlApiPdf + `anexo001_d27NT?id=${id}`, htmlOptions);
  }
