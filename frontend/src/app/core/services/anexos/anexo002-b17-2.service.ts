...
export class Anexo002B172Service {
  urlApi: string;
  urlAnexoRead: string;
  urlApiPdf: string;

  constructor(private httpClient: HttpClient) {
    this.urlApi = `${environment.baseUrlAPI}${environment.endPoint.anexos.anexo2_b172}/`;
  ...
  readPostFie(id: number): Observable<any> {
    ...
    return this.httpClient.get(this.urlApiPdf + `anexo002_b17_2?id=${id}`, htmlOptions);
  }
}
