...
export class Anexo004D27Service {
  urlApi: string = '';
  urlAnexoRead: string = '';
  urlApiPdf: string = '';

  constructor(private httpClient: HttpClient) {
    this.urlApi = `${environment.baseUrlAPI}${environment.endPoint.anexos.anexo4_d27}/`;
  ...
  readPostFie(id: number): Observable<any> {
    ...
    return this.httpClient.get(this.urlApiPdf + `anexo004_d27?id=${id}`, htmlOptions);
  }
