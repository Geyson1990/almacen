...
export class Anexo003B172Service {

  urlApi: string = '';
  urlApiPdf: string = '';

  constructor(private httpClient: HttpClient) {
    this.urlApi = `${environment.baseUrlAPI}${environment.endPoint.anexos.anexo3_b172}/`;
  ...
  readPostFie(id: number): Observable<any> {
    ...
    return this.httpClient.get(this.urlApiPdf + `anexo003_b172?id=${id}`, htmlOptions);
  }
}
