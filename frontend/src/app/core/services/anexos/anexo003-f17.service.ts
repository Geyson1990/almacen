...
export class Anexo003F17Service {
  urlApi: string = '';
  urlAnexoRead: string = '';
  urlApiPdf: string = '';

constructor(private httpClient: HttpClient) {
    this.urlApi = `${environment.baseUrlAPI}${environment.endPoint.anexos.anexo3_f17}/`;
  ...
  readPostFie(id: number): Observable<any> {
    ...
    return this.httpClient.get(this.urlApiPdf + `anexo003_f17?id=${id}`, htmlOptions);
  }
