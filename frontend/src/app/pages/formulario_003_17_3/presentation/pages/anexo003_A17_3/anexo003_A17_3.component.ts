...
import { Anexo003_A17_3Request, MetaData, Seccion2, Personal, Funcion } from '../../../domain/anexo003_A17_3/anexo003_A17_3Request';
...
import { Anexo003_A17_3Service } from '../../../application/usecases';
...
@Component({
	...
	selector: 'app-anexo003_A17_3',
	templateUrl: './anexo003_A17_3.component.html',
	styleUrls: ['./anexo003_A17_3.component.css']
})

// tslint:disable-next-line: class-name
export class Anexo003_A17_3_Component implements OnInit, AfterViewInit, AfterViewChecked {
	...
	txtTitulo = 'ANEXO 003-A/17.03 NOMINA DEL PERSONAL DE LA ENTIDAD CERTIFICADORA PARA EMITIR LOS CERTIFICADOS DE CONFORMIDAD DE VEHICULOS O DE LA ENTIDAD CERTIFICADORA DE VEHICULOS DE COLECCION';
	...
	listaFuncion: Funcion[] = [
		{ value: "1", text: 'Ingeniero mecánicos-electricistas' },
		{ value: "2", text: 'Ingeniero mecánicos y electricistas colegiados' },
		{ value: "3", text: 'Ingeniero certificador' },
	];
	...
	constructor(
		...
		private anexoService: Anexo003_A17_3Service,
	...
	ngOnInit(): void {
		...
		this.anexoFG = this.fb.group({
			a_Seccion1: this.fb.group({
				a_s1_declaracion_1: (this.codigoProcedimientoTupa === "DCV-009") ? [false] : [{ value: false, disabled: false }, []],
				a_s1_declaracion_2: (this.codigoProcedimientoTupa === "DCV-007") ? [false] : [{ value: false, disabled: false }, []],
			}),
	...
	async cargarDatos(): Promise<void> {
		...
		if (this.dataInput.movId > 0) {
			...
			try {
				const dataAnexo = await this.anexoTramiteService.get<Anexo003_A17_3Response>(this.dataInput.tramiteReqId).toPromise();
	...
	async descargarPdf(): Promise<void> {
		...
		try {
			...
			const urlPdf = URL.createObjectURL(file);
			modalRef.componentInstance.pdfUrl = urlPdf;
			modalRef.componentInstance.titleModal = 'Vista Previa - Anexo 003-A/17.03';
		}
		catch (e) {
			console.error(e);
			this.funcionesMtcService
	...
	agregarPersonal() {
		...
		if (this.indexEditTabla === -1) {
			...
			console.log("Tipo Documento: " + this.a_s2_tipoDocumento.value);
	...
	formularioCompleto() {
		if (this.a_s2_tipoDocumento.value == "" || this.a_s2_numeroDocumento.value == "" || this.a_s2_nombresApellidos.value == "" ||
			this.a_s2_funcion.value == "" || this.a_s2_cip.value == "") {
			console.log(this.a_s2_tipoDocumento.value);
			console.log(this.a_s2_numeroDocumento.value);
			console.log(this.a_s2_funcion.value);
			console.log(this.a_s2_nombresApellidos.value);
			console.log(this.a_s2_tipoDocumento.value);
			return false;
		}
		else
	...
	async guardarAnexo(): Promise<void> {
		...
		if(this.listaPersonal.length==0){
			this.funcionesMtcService.mensajeError('Debe ingresar al menos un personal de la nómina.');
			return;
		}

		const dataGuardar = new Anexo003_A17_3Request();
		// -------------------------------------
