...
import { Formulario001_PVIRequest } from 'src/app/pages/formulario_001_PVI/domain/Formulario001_PVIRequest';
import { Formulario001_PVIResponse } from 'src/app/pages/formulario_001_PVI/domain/Formulario001_PVIResponse';
import { MetaData } from 'src/app/pages/formulario_001_PVI/domain/MetaData';
import { Aeronave, InformacionVuelo } from 'src/app/pages/formulario_001_PVI/domain/Secciones';
import { TipoDocumentoModel } from 'src/app/core/models/TipoDocumentoModel';
import { Formulario001PVIService } from 'src/app/pages/formulario_001_PVI/application';
...
export class FormularioComponent implements OnInit, AfterViewInit {
  ...
  esVisibleCampoPermisoVuelo = false;
  ...
  txtTitulo = 'FORMULARIO 001-PVI SOLICITUD DE PERMISO PARA SOBREVUELO INTERNACIONAL';
  ...
  constructor(
    ...
    private formularioService: Formulario001PVIService,
  ...
  ngOnInit(): void {
    ...
    this.formularioFG = this.formBuilder.group({
      ...
      f_Seccion3FG: this.formBuilder.group({
        f_s3_TripulacionCalificada: [false, [Validators.requiredTrue]],
      }),
  ...
  tipoDeSolicitud(event: Event): void {
    ...
    if (selectedOption === '1') {
      ...
      this.esVisibleCampoPermisoVuelo = false;
      ...
      if (respuesta.ouT_FLG_RESULT == "1") {
        ...
        if (this.f_Seccion1FG.enabled) {
          ...
          if (this.f_s1_DatosSolicitudFG.enabled) {
            ...
            this.f_s1_sol_TipoPermisoFC.setValue('SOBREVUELO INTERNACIONAL');
  ...
  async obtenerListas(): Promise<void> {
    this.funcionesMtcService.mostrarCargando();
    this.listaPaises = await this.SiidgacService.getPaises().toPromise();
  ...
  async cargarDatos(): Promise<void> {
    ...
    if (this.dataInput != null && this.dataInput.movId > 0) {
      ...
      try {
        const dataFormulario = await this.formularioTramiteService.get<Formulario001_PVIResponse>(this.dataInput.tramiteReqId).toPromise();
        ...
        if (this.f_Seccion1FG.enabled) {
          ...
          if (this.f_s1_DatosSolicitudFG.enabled) {
            ...
            this.f_s1_sol_NumeroPasajerosFC.setValue(seccion1.datosSolicitud.numeroPasajero == 'TO BE ANNOUNCED' ? '' : seccion1.datosSolicitud.numeroPasajero);
  ...
  guardarFormulario(): void {
    ...
    const dataGuardar = new Formulario001_PVIRequest();
    dataGuardar.id = this.id;
    dataGuardar.codigo = 'F001-PVI';
    ...
    datosOperador.pais = this.listaPaises.find(item => item.idPais === datosOperador.idPais)?.descripcion;
    ...
    datosSolicitud.idAmbito = 'S5wX1p-D8ImAnQR8tdArPQ..';
    ...
    datosSolicitud.tipoVuelo = this.listaTipoVuelo.find(item => item.idMotivoVueloPorPersonaAmbito === datosSolicitud.idTipoVuelo)?.descripcionEspMotivo;
