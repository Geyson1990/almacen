/**
 * Formulario 004/12 utilizado por los procedimientos DGAC-034, DGAC-036, S-DGAC-003, S-DGAC-004, S-DGAC-005, S-DGAC-006 Y S-DGAC-007
...
import { Formulario004_12NTRequest } from 'src/app/core/models/Formularios/Formulario004_12NT/Formulario004_12NTRequest';
import { Formulario004_12NTResponse } from 'src/app/core/models/Formularios/Formulario004_12NT/Formulario004_12NTResponse';
import { Formulario00412NTService } from '../../../../core/services/formularios/formulario004-12NT.service';
...
export class FormularioComponent implements OnInit {
  ...
  //Datos de Formulario
  titulo = 'FORMULARIO 004/12 SEGURIDAD DE VUELO';
  tipoPersona: number = 1;

  paSeccion1: string[] = ["DGAC-034", "DGAC-036", "S-DGAC-003", "S-DGAC-004", "S-DGAC-005", "S-DGAC-006", "S-DGAC-007"];
  paSeccion11: string[] = ["DGAC-034"];
  paSeccion12: string[] = ["DGAC-036"];
  paSeccion2: string[] = ["DGAC-034", "DGAC-036", "S-DGAC-003", "S-DGAC-004", "S-DGAC-005", "S-DGAC-006", "S-DGAC-007"];
  paSeccion3: string[] = ["DGAC-034", "DGAC-036", "S-DGAC-003", "S-DGAC-004", "S-DGAC-005", "S-DGAC-006", "S-DGAC-007"];
  paSeccion4: string[] = ["DGAC-034", "DGAC-036", "S-DGAC-003", "S-DGAC-004", "S-DGAC-005", "S-DGAC-006", "S-DGAC-007"];
  ...
  paDJ1: string[] = ["DGAC-034", "DGAC-036", "S-DGAC-003", "S-DGAC-004", "S-DGAC-005", "S-DGAC-006", "S-DGAC-007"];
  paDJ2: string[] = ["DGAC-034", "DGAC-036", "S-DGAC-003", "S-DGAC-004", "S-DGAC-005", "S-DGAC-006", "S-DGAC-007"];
  ...
  constructor(
    ...
    private formularioService: Formulario00412NTService,
  ...
  ngOnInit(): void {
    ...
    this.formulario = this.fb.group({
      ...
      memoria: this.fb.control(''),
      estudio: this.fb.control(''),
      oficio_dgac: this.fb.control(""),
      ...
      telefonoPersona: this.fb.control("", [Validators.maxLength(11)]),
    ...
    if(this.codigoProcedimientoTupa === "DGAC-036")
      this.formulario.controls['oficio_dgac'].setValidators([Validators.required]);
  ...
  cargarDatos() {
    ...
    if (this.dataInput != null && this.dataInput.movId > 0) {
      ...
      this.formularioTramiteService.get<any>(this.dataInput.tramiteReqId).subscribe(
        (dataFormulario: Formulario004_12NTResponse) => {
          ...
          if (this.habilitarSeccion11) {
            this.formulario.controls['memoria'].setValue(metaData.seccion1.memoria ?? '');
            this.formulario.controls['estudio'].setValue(metaData.seccion1.estudio ?? '');
            this.pathPdfMemoriaDescriptivaSeleccionado = metaData.seccion1.pathNameMemoriaDescriptiva ?? null;
            this.pathPdfImpactoAmbientalSeleccionado = metaData.seccion1.pathNameImpactoAmbiental ?? null;
          }
      ...
      if (this.activarPN) {
        ...
        console.log("this.nroDocumentoLogin", this.nroDocumentoLogin)
        console.log("getCompanyCode",  this.seguridadService.getCompanyCode())
  ...
  onChangeMemoriaDescriptiva() {
    ...
    this.visibleButtonMemoriaDescriptiva = this.formulario.controls['memoria']?.value ?? false;
    ...
    if (this.visibleButtonMemoriaDescriptiva === true) {
      this.funcionesMtcService.mensajeConfirmar('¿Declaro que la información adjunta en el archivo es verídica?', 'DECLARACIÓN').catch(() => {
        ...
        this.formulario.controls['memoria'].setValue(false);
      });
    } else {
  ...
  onChangeImpactoAmbiental() {
    ...
    this.visibleButtonImpactoAmbiental = this.formulario.controls['estudio']?.value ?? false;

    if (this.visibleButtonImpactoAmbiental === true) {
      this.funcionesMtcService.mensajeConfirmar('¿Declaro que la información adjunta en el archivo es verídica?', 'DECLARACIÓN').catch(() => {
        this.visibleButtonImpactoAmbiental = false;
        this.formulario.controls['estudio'].setValue(false);
      });
    } else {
  ...
  vistaPreviaMemoriaDescriptiva() {
    ...
    if (this.pathPdfMemoriaDescriptivaSeleccionado === null || this.filePdfMemoriaDescriptivaSeleccionado !== null) {
      ...
      modalRef.componentInstance.titleModal = "Vista Previa - Reparaciones Mayores";
    } else {
      this.funcionesMtcService.mostrarCargando();
  ...
  vistaPreviaImpactoAmbiental() {
    if (this.pathPdfImpactoAmbientalSeleccionado === null || this.filePdfImpactoAmbientalSeleccionado !== null) {
      ...
      modalRef.componentInstance.pdfUrl = urlPdf;
      modalRef.componentInstance.titleModal = "Vista Previa - Alteraciones Mayores";
    } else {
      this.funcionesMtcService.mostrarCargando();
  ...
  guardarFormulario() {
    ...
    if (this.codigoProcedimientoTupa === 'DGAC-034') {
      if (!this.filePdfMemoriaDescriptivaSeleccionado
        && !this.pathPdfMemoriaDescriptivaSeleccionado
        && !this.filePdfImpactoAmbientalSeleccionado
        && !this.pathPdfImpactoAmbientalSeleccionado) {
        return this.funcionesMtcService.mensajeError('Debe ingresar alguno de los documentos de datos de mantenimiento.');
      }
    }

    let dataGuardar: Formulario004_12NTRequest = new Formulario004_12NTRequest();
    let form = this.formulario.controls;

    dataGuardar.id = this.id;
    dataGuardar.codigo = 'F004-12';
    ...
    dataGuardar.metaData.seccion1.dgac_034 = (this.codigoProcedimientoTupa === "DGAC-034" ? "1" : "0");
    dataGuardar.metaData.seccion1.dgac_036 = (this.codigoProcedimientoTupa === "DGAC-036" ? "1" : "0");
    dataGuardar.metaData.seccion1.s_dgac_003 = (this.codigoProcedimientoTupa === "S-DGAC-003" ? "1" : "0");
    dataGuardar.metaData.seccion1.s_dgac_004 = (this.codigoProcedimientoTupa === "S-DGAC-004" ? "1" : "0");
    dataGuardar.metaData.seccion1.s_dgac_005 = (this.codigoProcedimientoTupa === "S-DGAC-005" ? "1" : "0");
    dataGuardar.metaData.seccion1.s_dgac_006 = (this.codigoProcedimientoTupa === "S-DGAC-006" ? "1" : "0");
    dataGuardar.metaData.seccion1.s_dgac_007 = (this.codigoProcedimientoTupa === "S-DGAC-007" ? "1" : "0");
    dataGuardar.metaData.seccion1.memoria = form['memoria'].value;
    dataGuardar.metaData.seccion1.estudio = form['estudio'].value;
