...
import { MetaData, Formulario002_12Response, Formulario002_12Request, Tripulante } from '../../../domain';
import { Formulario00212Service } from '../../../application';
...
export class FormularioComponent implements OnInit {
  ...
  formAgregarTripulante: UntypedFormGroup;
  listaTripulantes: Array<Tripulante> = []
  ...
  habilitarBusquedaTripulante: boolean = false
  ...
  // txtTitulo = 'FORMULARIO 002/12 - PERSONAL AERONAÚTICO - Evaluación de personal de vuelo internacional';
  txtTitulo = 'FORMULARIO 002/12 - PERSONAL AERONAÚTICO - Evaluación de personal de vuelo internacional';
  ...
  constructor(
    ...
    private formularioService: Formulario00212Service,
    ...
    private renatService : RenatService
      ...
      console.log("tramiteSelected", tramiteSelected)
      ...
      // PRUEBAS RENAT
      // this.renatService.validarPlacaPerteneceFlotaVehicularEmpresa("string", 1, "string").subscribe((response) => {
      //   console.log("validarPlacaPerteneceFlotaVehicularEmpresa", response)
      // })

      // this.renatService.validarConductorPerteneceNominaConductores("string", "string", "string").subscribe((response) => {
      //   console.log("validarConductorPerteneceNominaConductores", response)
      // })

      // this.renatService.validarPlacaPerteneceRutaEmpresa("string", "string", "string").subscribe((response) => {
      //   console.log("validarPlacaPerteneceRutaEmpresa", response)
      // })
  }

  ngOnInit(): void {
    console.log("this.dataInput", this.dataInput)
    console.log("this.dataRequisitosInput", this.dataRequisitosInput)
    ...
    this.setFormAgregarTripulante();

    if (this.dataInput != null && this.id > 0) {
        this.funcionesMtcService.mostrarCargando();
        this.formularioTramiteService.get<Formulario002_12Response>(this.tramiteReqId)
        .subscribe((response) => {
          this.funcionesMtcService.ocultarCargando();
          this.id = response.formularioId;
          const metaData = JSON.parse(response.metaData) as MetaData;

          this.FlagTripulantesTecnicos.setValue(metaData.seccion1.flagTripulantesTecnicos);
          this.FlagTripulantesAuxiliares.setValue(metaData.seccion1.flagTripulantesAuxiliares);
          if (this.solicitaPN) {
          ...
          this.listaTripulantes = metaData.seccion3.tripulantes
  ...
  setFormPN() {
    this.formulario = this.formBuilder.group({
      FlagTripulantesTecnicos: [false],
      FlagTripulantesAuxiliares: [false],
  ...
  setFormAgregarTripulante() {
    this.formAgregarTripulante = this.formBuilder.group({
      tipoDocIdentidad: ["", [Validators.required]],
      nroDocIdentidad: ["", [Validators.required]],
      apellidoPaterno: ["", [Validators.required]],
      apellidoMaterno: ["", [Validators.required]],
      nombre: ["", [Validators.required]],
      tipoEvaluacionEquipo: ["", [Validators.required]],
    });
  }

  get FlagTripulantesTecnicos(): AbstractControl { return this.formulario.get(['FlagTripulantesTecnicos']); }
  get FlagTripulantesAuxiliares(): AbstractControl { return this.formulario.get(['FlagTripulantesAuxiliares']); }
  ...
  get TipoDocIdentidadTripulante(): AbstractControl { return this.formAgregarTripulante.get(['tipoDocIdentidad']); }
  get NroDocIdentidadTripulante(): AbstractControl { return this.formAgregarTripulante.get(['nroDocIdentidad']); }
  get ApellidoPaternoTripulante(): AbstractControl { return this.formAgregarTripulante.get(['apellidoPaterno']); }
  get ApellidoMaternoTripulante(): AbstractControl { return this.formAgregarTripulante.get(['apellidoMaterno']); }
  get NombreTripulante(): AbstractControl { return this.formAgregarTripulante.get(['nombre']); }
  get TipoEvaluacionEquipoTripulante(): AbstractControl { return this.formAgregarTripulante.get(['tipoEvaluacionEquipo']); }
  ...
  onInputDocTripulante(){
    console.dir('onInputNroDoc')
    this.habilitarBusquedaTripulante = true
    this.TipoEvaluacionEquipoTripulante.setValue("");
    this.NombreTripulante.setValue("");
    this.ApellidoPaternoTripulante.setValue("");
    this.ApellidoMaternoTripulante.setValue("");
  }

  agregarTripulante(){
    if (this.formAgregarTripulante.invalid)
      return this.formAgregarTripulante.markAllAsTouched();

    this.listaTripulantes.push({
      tipoDocIdentidad: this.TipoDocIdentidadTripulante.value == '01'? "D.N.I." : "C.E.",
      docIdentidad: this.NroDocIdentidadTripulante.value,
      nombres: this.NombreTripulante.value,
      apellidoPaterno: this.ApellidoPaternoTripulante.value,
      apellidoMaterno: this.ApellidoMaternoTripulante.value,
      tipoEvaluacionEquipo: this.TipoEvaluacionEquipoTripulante.value,
    } as Tripulante)
    this.formAgregarTripulante.reset()
    this.TipoDocIdentidadTripulante.setValue("")
  }

  eliminarTripulante(index: number) {
    this.funcionesMtcService.mensajeConfirmar('¿Está seguro de eliminar el personal?', 'Eliminar')
    .then(() => {
      this.listaTripulantes.splice(index, 1);
    });
  }

  onPressEnterDocTripulante(event:any, tipoDoc:string, doc:string) {
    var charCode = (event.which) ? event.which : event.keyCode;
    if (charCode === 13)
      this.buscarDocTripulante(tipoDoc, doc)
  }

  buscarDocTripulante(tipoDocumento: string, numeroDocumento:string){

    if(!this.habilitarBusquedaTripulante)
      return;
    if (tipoDocumento.length === 0 || numeroDocumento.length === 0)
      return this.funcionesMtcService.mensajeError('Debe ingresar Tipo de Documento y/o Número de Documento');
    if (tipoDocumento === '01' && numeroDocumento.length !== 8)
      ...
      return this.funcionesMtcService.mensajeError('C.E. debe tener 9 caracteres');

    this.funcionesMtcService.mostrarCargando();

    if (tipoDocumento === '01') {// DNI
      this.reniecService.getDni(numeroDocumento).subscribe((response) => {
        this.habilitarBusquedaTripulante = false
        this.funcionesMtcService.ocultarCargando();
        ...
        console.log("prenombres", prenombres)

        this.NombreTripulante.setValue(prenombres)
        this.ApellidoPaternoTripulante.setValue(apPrimer)
        this.ApellidoMaternoTripulante.setValue(apSegundo)

      }, (error) => {
        this.funcionesMtcService.ocultarCargando().mensajeError('Error al consultar el servicio de Reniec');
        this.habilitarBusquedaTripulante = false
      });
    } else if (tipoDocumento === '04') {// CARNÉ DE EXTRANJERÍA
      this.extranjeriaService.getCE(numeroDocumento).subscribe((response) => {
        ...
        const {nombres, primerApellido, segundoApellido} = response.CarnetExtranjeria
        this.NombreTripulante.setValue(nombres)
        this.ApellidoPaternoTripulante.setValue(primerApellido)
        this.ApellidoMaternoTripulante.setValue(segundoApellido)
  ...
  guardarFormulario() {
    ...
    if(!this.FlagTripulantesAuxiliares.value && !this.FlagTripulantesTecnicos.value)
      return this.funcionesMtcService.mensajeError('Seleccione una opción de tripulantes');

    if(this.listaTripulantes.length === 0)
      return this.funcionesMtcService.mensajeError('Complete la sección "Datos del solicitante"');

    const dataGuardar = new Formulario002_12Request();

    dataGuardar.id = this.id;
    dataGuardar.codigo = 'F002-12';
    ...
    dataGuardar.metaData.seccion1.flagTripulantesTecnicos = this.FlagTripulantesTecnicos.value
    dataGuardar.metaData.seccion1.flagTripulantesAuxiliares = this.FlagTripulantesAuxiliares.value
    dataGuardar.metaData.seccion2.modalidadNotificacion = this.ModalidadNotificacion.value

    dataGuardar.metaData.seccion3.tripulantes       = this.listaTripulantes
  ...
  async descargarPdf(): Promise<void> { // OK
    ...
    try {
      ...
      modalRef.componentInstance.titleModal = 'Vista Previa - Formulario 002/12';
    }
    catch (e) {
