/**
 * Formulario 001/03 utilizado por los procedimientos PRONATEL-001 y DGPRC-001
 * @author Mackenneddy Meléndez Coral
 * @version 1.0 05.10.2022
*/
import { Component, OnInit, Input, ViewChild, AfterViewInit } from '@angular/core';
import { UntypedFormGroup, UntypedFormBuilder, Validators, AbstractControl, FormArray, UntypedFormControl, ValidationErrors } from '@angular/forms';
...
// import { Formulario001_03Request } from 'src/app/core/models/Formularios/Formulario001_03/Formulario001_03Request';
// import { Formulario001_03Response } from 'src/app/core/models/Formularios/Formulario001_03/Formulario001_03Response';
// import { Formulario00103Service } from '../../../../core/services/formularios/formulario001-03.service';
...
import { FuncionesMtcService } from '../../../../../core/services/funciones-mtc.service';
import { SeguridadService } from '../../../../../core/services/seguridad.service';
import { Formulario001_03Service } from '../../../application/usecases';
import { VisorPdfArchivosService } from '../../../../../core/services/tramite/visor-pdf-archivos.service';
...
export class FormularioComponent implements OnInit, AfterViewInit {
  ...
  txtTituloCompleto: string = "FORMULARIO 001/03 DECLARACIÓN DE CONFIDENCIALIDAD DE LA INFORMACIÓN";
  ...
  //Datos de Formulario
  tituloFormulario = 'DECLARACIÓN DE CONFIDENCIALIDAD DE LA INFORMACIÓN';
  tipoPersona: number = 1;

  paSeccion141: string[] = ["PRONATEL-001"];
  habilitarSeccion141: boolean = true;

  paSeccion142: string[] = ["DGPRC-001"];
  habilitarSeccion142: boolean = true;


  paDJ1: string[] = ["PRONATEL-001", "DGPRC-001"];
  paDJ2: string[] = ["PRONATEL-001", "DGPRC-001"];
  ...
  constructor(
    ...
    private formularioService: Formulario001_03Service,
  ...
  async ngOnInit(): Promise<void> {
    ...
    console.log("Codigo Procedimiento PA:", this.paSeccion141.indexOf(this.codigoProcedimientoTupa));

    if (this.paSeccion141.indexOf(this.codigoProcedimientoTupa) > -1) this.habilitarSeccion141 = true; else this.habilitarSeccion141 = false;
    if (this.paSeccion142.indexOf(this.codigoProcedimientoTupa) > -1) this.habilitarSeccion142 = true; else this.habilitarSeccion142 = false;
    ...
    this.formulario = this.fb.group({
      ...
      idDocDeclaConfi: this.fb.control("", [Validators.required]),
      listadoDeclaConfi: this.fb.control("", [Validators.required]),
      razonesDeclaConfi: this.fb.control("", [Validators.required]),
      


      periodoDeclaConfi: (this.codigoProcedimientoTupa === 'PRONATEL-001') ?this.fb.control({value:'',disabled:true}, []):"",
      periodoDeclaConfiFechaIni: (this.codigoProcedimientoTupa === 'PRONATEL-001') ?this.fb.control("", [Validators.required,this.isFecha]):"",
      periodoDeclaConfiFechaFin: (this.codigoProcedimientoTupa === 'PRONATEL-001') ?this.fb.control("", [Validators.required,this.isFecha]):"",

      periodoIsActivoCalc: (this.codigoProcedimientoTupa === 'PRONATEL-001') ? this.fb.control(false) : "false",


      periodoDecla: (this.codigoProcedimientoTupa === 'DGPRC-001') ? this.fb.control("", [Validators.required, Validators.minLength(2)]) : "",
      ...
      validators:[this.validarRangoFechas('periodoDeclaConfiFechaIni','periodoDeclaConfiFechaFin')]
    });
  }
  ...
  // check
  isActivoCalCuloPeriodo( ){

    const isChek=  this.f_periodoIsActivoCalc.value  ;
    
    if(isChek){
      this.f_periodoDeclaConfiFechaFin.setValue(null);     
      this.f_periodoDeclaConfiFechaFin.disable(); 

      this.f_periodoDeclaConfi.setValue('-');      

      this.f_periodoDeclaConfiFechaFin.clearValidators();
      this.f_periodoDeclaConfiFechaFin.updateValueAndValidity();

      this.formulario.clearValidators();
      this.formulario.updateValueAndValidity();
    }else{
      this.f_periodoDeclaConfiFechaFin.enable();      
      this.f_periodoDeclaConfi.setValue('');  
      this.f_periodoDeclaConfiFechaFin.setValidators([Validators.required,this.isFecha]);
      this.f_periodoDeclaConfiFechaFin.updateValueAndValidity();
      this.formulario.setValidators([this.validarRangoFechas('periodoDeclaConfiFechaIni','periodoDeclaConfiFechaFin')]);
      this.formulario.updateValueAndValidity();
    }    
  ...
  //--- Validadores 
  validarRangoFechas(fechaini:string,fechaend:string){
    return (formGroup:AbstractControl) : ValidationErrors | null=>{

      const fini = Date.parse(formGroup.get(fechaini)?.value);
      const fend= Date.parse(formGroup.get(fechaend)?.value);

      if(fini>=fend){
        return {
          isNotValidRango:true
        } ;
      }    
      return null;
    }
  ...
  isFecha(control:UntypedFormControl){
  ...
  get f_idDocDeclaConfi(): AbstractControl { return this.formulario.get(['idDocDeclaConfi']); }
  get f_listadoDeclaConfi(): AbstractControl { return this.formulario.get(['listadoDeclaConfi']); }
  get f_razonesDeclaConfi(): AbstractControl { return this.formulario.get(['razonesDeclaConfi']); }
  get f_periodoDeclaConfi(): AbstractControl { return this.formulario.get(['periodoDeclaConfi']); }

  get f_periodoDeclaConfiFechaIni(): AbstractControl { return this.formulario.get(['periodoDeclaConfiFechaIni']); }
  get f_periodoDeclaConfiFechaFin(): AbstractControl { return this.formulario.get(['periodoDeclaConfiFechaFin']); }

  get f_periodoIsActivoCalc(): AbstractControl { return this.formulario.get(['periodoIsActivoCalc']); }

  get f_periodoDecla(): AbstractControl { return this.formulario.get(['periodoDecla']); }
  ...
  async cargarDatos(): Promise<void> {
    ...
    if (this.dataInput != null && this.dataInput.movId > 0) {
      try {
        const dataFormulario = await this.formularioTramiteService.get<Formulario001_03Response>(this.dataInput.tramiteReqId).toPromise();
        this.funcionesMtcService.ocultarCargando();
        const metaData = JSON.parse(dataFormulario.metaData);
        console.log(metaData);
        this.id = dataFormulario.formularioId;

        this.f_idDocDeclaConfi.setValue(metaData.seccion1.idDocDeclaConfi);
        this.f_listadoDeclaConfi.setValue(metaData.seccion1.listadoDeclaConfi);
        this.f_razonesDeclaConfi.setValue(metaData.seccion1.razonesDeclaConfi);
        this.f_periodoDeclaConfi.setValue(metaData.seccion1.periodoDeclaConfi);
        this.f_periodoDeclaConfiFechaIni.setValue(metaData.seccion1.periodoDeclaConfiFechaIni);
        this.f_periodoDeclaConfiFechaFin.setValue(metaData.seccion1.periodoDeclaConfiFechaFin);
        this.f_periodoIsActivoCalc.setValue(metaData.seccion1.periodoIsActivoCalc);

        this.f_periodoDecla.setValue(metaData.seccion1.periodoDecla);
      ...
      this.isActivoCalCuloPeriodo();
      if(this.codigoProcedimientoTupa === 'PRONATEL-001'){
        this.calPeriodo();
      }
  ...
  guardarFormulario() {
    ...
    let dataGuardar: Formulario001_03Request = new Formulario001_03Request();

    dataGuardar.id = this.id;
    dataGuardar.codigo = 'F001-03';
    ...
    dataGuardar.metaData.seccion1.idDocDeclaConfi = this.f_idDocDeclaConfi.value;
    dataGuardar.metaData.seccion1.listadoDeclaConfi = this.f_listadoDeclaConfi.value;
    dataGuardar.metaData.seccion1.razonesDeclaConfi = this.f_razonesDeclaConfi.value;

    dataGuardar.metaData.seccion1.periodoDeclaConfi = this.f_periodoDeclaConfi.value;
dataGuardar.metaData.seccion1.periodoDeclaConfiFechaIni = this.f_periodoDeclaConfiFechaIni.value;
dataGuardar.metaData.seccion1.periodoDeclaConfiFechaFin = this.f_periodoDeclaConfiFechaFin.value;
 dataGuardar.metaData.seccion1.periodoIsActivoCalc = this.f_periodoIsActivoCalc.value;

 dataGuardar.metaData.seccion1.periodoDecla = this.f_periodoDecla.value;
    ...
    dataGuardar.metaData.seccion5.documento = this.listaTiposDocumentos.filter(item => item.id == this.codTipoDocSolicitante)[0].documento; 
    dataGuardar.metaData.seccion5.numeroDocumentoSolicitante = this.f_NroDocumentoSolicitante.value;
    dataGuardar.metaData.seccion5.nombreSolicitante = this.nombreUsuario;
    ...
    this.visorPdfArchivosService.get(this.uriArchivo)
      ...
      .subscribe(
        (file: Blob) => {
          ...
          modalRef.componentInstance.titleModal = "Vista Previa - Formulario 001/03";
  ...
  calPeriodo(){    
    ...
    let iniF=this.f_periodoDeclaConfiFechaIni.value ;
    let finF=this.f_periodoDeclaConfiFechaFin.value ;
    if(!iniF || !finF ){
      return ;
    }
    let stringTime="";
    if(Date.parse(iniF)>=Date.parse(finF)){
      stringTime="";
    }else{

      let r=this.calcularPeriodo(iniF,finF);
     
       if(r!.anio > 0){
          let tx=(r!.anio>1) ? "años":"año";
          stringTime+=` ${r!.anio} ${tx}`;
       } 
  
       if(r!.meses > 0){
        let tx=(r!.meses>1) ? "meses":"mes";
        let c=(r!.anio > 0) ? " ,":"";
        stringTime+=`${c} ${r!.meses} ${tx}`;
      }
  
      if(r!.dias > 0){
        let tx=(r!.dias>1) ? "días":"día";
        let cc="";
        if(r!.meses > 0 || r!.anio>0 ){
          cc=" y";
        }  
  
        stringTime+=`${cc} ${r!.dias} ${tx}  `;
     }  
   ...
   this.f_periodoDeclaConfi.setValue(stringTime);

  }

  
  calcularPeriodo(fechaini:any,fechaend:any) {
    if(!fechaini || !fechaend ){
        return ;
    }
    var hoy = new Date(fechaini);
    var fecha_ = new Date(fechaend);
    var edadTime = hoy.getTime() - fecha_.getTime();
    var d = this.dateDiff(fecha_, hoy);
    return {
      anio: d.years,
      meses:d.months,
      dias:d.days
     ...
     dateDiff(dt1:any, dt2:any) {
      if( ! (dt1 instanceof Date))
        dt1 = this.parseDate(dt1);
      
      if( ! (dt2 instanceof Date))
        dt2 = this.parseDate(dt2);
     ...
     parseDate(txtDate:any) {
      if(txtDate) {
        if(typeof txtDate === "object" && typeof txtDate.getTime === 'function')
          return txtDate;        
