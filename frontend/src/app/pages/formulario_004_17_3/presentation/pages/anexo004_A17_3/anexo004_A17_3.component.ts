...
import { Anexo004_A17_3Request, MetaData, Seccion2, Personal, Funcion } from '../../../domain/anexo004_A17_3/anexo004_A17_3Request';
...
import { Anexo004_A17_3Service } from '../../../application/usecases';
...
@Component({
	...
	selector: 'app-anexo004_A17_3',
	templateUrl: './anexo004_A17_3.component.html',
	styleUrls: ['./anexo004_A17_3.component.css']
})

// tslint:disable-next-line: class-name
export class Anexo004_A17_3_Component implements OnInit, AfterViewInit, AfterViewChecked {
	...
	txtTitulo = 'ANEXO 004-A/17.03 NOMINA DEL PERSONAL DE LA ENTIDAD CERTIFICADORA DE CONVERSIONES A GAS LICUADO DE PETROLEO-GLP O ENTIDAD CERTIFICADORA DE CONVERSIONES A GAS NATURAL VEHICULAR-GNV';
   ...
   pathPdfDocumentosSeleccionado: any = null;
	...
	listaFuncion: Funcion[] = [
		...
		{ value: "3", text: 'Personal técnico especializado' }
	];
	...
	dj_GLP: string[] = ["DCV-013", "DCV-014"];
	dj_GNV: string[] = ["DCV-011", "DCV-012"];

	activarGLP: boolean = true;
	activarGNV: boolean = true;

	activarDocumentos: boolean = true;
   paDocumentos: string[] = ["DCV-011","DCV-013"];
	...
	constructor(
		...
		private anexoService: Anexo004_A17_3Service,
	...
	ngOnInit(): void {
		...
		if (this.activarGNV) {
			this.listaFuncion = this.listaFuncion.filter(i => i.value !== "2");
		}

		if (this.activarGLP) {
			this.listaFuncion = this.listaFuncion.filter(i => i.value !== "3");
		}
		...
		this.anexoFG = this.fb.group({
			...
			a_Seccion2: this.fb.group({
				...
				a_s2_funcion: [''],
				a_s2_especialidad: ['',[Validators.maxLength(100)]],
				a_s2_cip: ['', [Validators.maxLength(10)]]
			}),
		});
	}
	...
	get a_s2_especialidad(): UntypedFormControl { return this.a_Seccion2.get('a_s2_especialidad') as UntypedFormControl }
	...
	async cargarDatos(): Promise<void> {
		...
		if (this.dataInput.movId > 0) {
			...
			try {
				const dataAnexo = await this.anexoTramiteService.get<Anexo004_A17_3Response>(this.dataInput.tramiteReqId).toPromise();
	...
	async descargarPdf(): Promise<void> {
		...
		try {
			...
			const urlPdf = URL.createObjectURL(file);
			modalRef.componentInstance.pdfUrl = urlPdf;
			modalRef.componentInstance.titleModal = 'Vista Previa - Anexo 004-A/17.03';
		}
		catch (e) {
			console.error(e);
			this.funcionesMtcService
	...
	changeTipoDocumento() {
		...
		this.a_s2_especialidad.setValue('');
	...
	agregarPersonal() {
		...
		if (this.a_s2_funcion.value == "1" && this.a_s2_cip.value == "") {
			return this.funcionesMtcService.mensajeError('Debe indicar el número de CIP del Ingeniero Supervisor');
		}
		...
		if (this.indexEditTabla === -1) {
			...
			this.listaPersonal.push({
				...
				especialidad: this.a_s2_especialidad.value,
			...
			this.listaPersonal[this.indexEditTabla].especialidad = this.a_s2_especialidad.value;
	...
	formularioCompleto() {
		if (this.a_s2_tipoDocumento.value == "" || this.a_s2_numeroDocumento.value == "" || this.a_s2_nombresApellidos.value == "" ||
			this.a_s2_funcion.value == "" || this.a_s2_especialidad.value == "") {
	...
	modificarPersonal(item: Personal, index) {
		...
		this.a_s2_especialidad.setValue(item.especialidad);
   ...
   vistaPreviaDocumentos(): void {
	...
	async guardarAnexo(): Promise<void> {
		...
		let cantSupervisor = this.listaPersonal.filter(item => item.funcion.value=="1");
		let cantInspector = this.listaPersonal.filter(item => item.funcion.value=="2");
		let cantTecnico = this.listaPersonal.filter(item => item.funcion.value=="3");

		if(cantSupervisor.length == 0){
			return this.funcionesMtcService.mensajeError('Debe ingresar al menos un Ingeniero Supervisor de la Entidad Certificadora');
		}

		if(this.activarGLP && cantInspector.length == 0){
			return this.funcionesMtcService.mensajeError('Debe ingresar al menos un Inspector de la Entidad Certificadora');
		}

		if(this.activarGNV && cantTecnico.length == 0){
			return this.funcionesMtcService.mensajeError('Debe ingresar al menos un Personal Técnico Especializado de la Entidad Certificadora');
		}

		const dataGuardar = new Anexo004_A17_3Request();
		// -------------------------------------
