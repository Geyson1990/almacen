/**
 * Formulario 001/17.03 utilizado por los procedimientos DCV-001, DCV-002, DCV-003
 * @author Mackenneddy Meléndez Coral
 * @version 1.0 26.10.2022
 * Modificado por Alicia Toquila
...
import { Formulario001_17_03Request, DatosContacto } from '../../../domain/formulario001_17_03/formulario001_17_03Request';
import { Formulario001_17_03Response } from '../../../domain/formulario001_17_03/formulario001_17_03Response';
import { Formulario001_17_03Service } from '../../../application/usecases';
...
export class FormularioComponent implements OnInit, AfterViewInit {
   ...
   txtTituloCompleto: string = "FORMULARIO 001/17.03 OTORGAMIENTO O RENOVACIÓN DE LA LICENCIA PARA CONDUCIR VEHÍCULOS FERROVIARIOS / OTORGAMIENTO DE LA LC VEHÍCULOS FERROVIARIOS DE CATEGORÍA ESPECIAL";
   ...
   //Datos de Formulario
   tituloFormulario = 'OTORGAMIENTO O RENOVACIÓN DE LA LICENCIA PARA CONDUCIR VEHÍCULOS FERROVIARIOS / OTORGAMIENTO DE LA LC VEHÍCULOS FERROVIARIOS DE CATEGORÍA ESPECIAL';
   tipoPersona: number = 1;


   paDJ1: string[] = ["DCV-001", "DCV-002"];
   paDJ2: string[] = ["DCV-001", "DCV-002"];
   ...
   constructor(
      ...
      private formularioService: Formulario001_17_03Service,
      ...
      this.formulario = this.fb.group({
         ...
         Seccion3: this.fb.group({
            PersonaNatural: this.fb.group({
               ...
               pn_telefono: ['', [Validators.maxLength(12)]],
            ...
            RepresentanteLegal: this.fb.group({
               ...
               rl_apeMaterno: [{value:'', disabled:true}, [Validators.maxLength(30)]],
            ...
            DatosContacto: this.fb.group({
               ...
               dc_apeMaterno: [{value:'', disabled:true}, [Validators.maxLength(30)]],
               dc_telefono: ['', [Validators.maxLength(20)]],
               dc_celular: ['', [Validators.required, Validators.maxLength(9)]],
               dc_correo: ['', [Validators.required, Validators.email, noWhitespaceValidator(), Validators.maxLength(50)]]
            })
         }),
   ...
   get f_s3_DatosContacto(): UntypedFormGroup { return this.f_Seccion3.get('DatosContacto') as UntypedFormGroup; }
   get f_s3_dc_TipoDocumento(): AbstractControl { return this.f_s3_DatosContacto.get(['dc_tipoDocumento']); }
   get f_s3_dc_NumeroDocumento(): AbstractControl { return this.f_s3_DatosContacto.get(['dc_numeroDocumento']); }
   get f_s3_dc_Nombre(): AbstractControl { return this.f_s3_DatosContacto.get(['dc_nombre']); }
   get f_s3_dc_ApePaterno(): AbstractControl { return this.f_s3_DatosContacto.get(['dc_apePaterno']); }
   get f_s3_dc_ApeMaterno(): AbstractControl { return this.f_s3_DatosContacto.get(['dc_apeMaterno']); }
   get f_s3_dc_Telefono(): AbstractControl { return this.f_s3_DatosContacto.get(['dc_telefono']); }
   get f_s3_dc_Celular(): AbstractControl { return this.f_s3_DatosContacto.get(['dc_celular']); }
   get f_s3_dc_Correo(): AbstractControl { return this.f_s3_DatosContacto.get(['dc_correo']); }
   ...
   async buscarNumeroDocumentoDatosContacto(): Promise<void> {
      const tipoDocumento: string = this.f_s3_dc_TipoDocumento.value.trim();
      const numeroDocumento: string = this.f_s3_dc_NumeroDocumento.value.trim();
      ...
      if (tipoDocumento === '01') {// DNI
         try {
            ...
            this.funcionesMtcService.ocultarCargando().mensajeError('Error al consultar al servicio');
            this.f_s3_dc_Nombre.enable();
            this.f_s3_dc_ApePaterno.enable();
            this.f_s3_dc_ApeMaterno.enable();
         }
      } else if (tipoDocumento === '04') {// CARNÉ DE EXTRANJERÍA
         try {
            const respuesta = await this.extranjeriaService.getCE(numeroDocumento).toPromise();
   ...
   async addContacto(
      ...
      this.funcionesMtcService
         .mensajeConfirmar(`Los datos ingresados fueron validados por ${tipoDocumento === '01' ? 'RENIEC' : 'MIGRACIONES'} corresponden a la persona ${nombres} ${apPaterno} ${apMaterno}. ¿Está seguro de agregarlo?`)
         .then(async () => {
            this.f_s3_dc_Nombre.setValue(nombres);
            this.f_s3_dc_ApePaterno.setValue(apPaterno);
            this.f_s3_dc_ApeMaterno.setValue(apMaterno);


            this.f_s3_dc_Nombre.disable({ emitEvent: false });
            this.f_s3_dc_ApePaterno.disable({ emitEvent: false });
            this.f_s3_dc_ApeMaterno.disable({ emitEvent: false });
   ...
   async cargarDatos(): Promise<void> {
      ...
      if (this.dataInput != null && this.dataInput.movId > 0) {
         try {
            const dataFormulario = await this.formularioTramiteService.get<Formulario001_17_03Response>(this.dataInput.tramiteReqId).toPromise();
            this.funcionesMtcService.ocultarCargando();
            ...
            this.f_s3_dc_Nombre.setValue(metaData.seccion3.DatosContacto.nombres);
            this.f_s3_dc_ApePaterno.setValue(metaData.seccion3.DatosContacto.apellidoPaterno);
            this.f_s3_dc_ApeMaterno.setValue(metaData.seccion3.DatosContacto.apellidoMaterno);
            this.f_s3_dc_TipoDocumento.setValue(metaData.seccion3.DatosContacto.tipoDocumento.id);
            this.f_s3_dc_NumeroDocumento.setValue(metaData.seccion3.DatosContacto.numeroDocumento);
            this.f_s3_dc_Telefono.setValue(metaData.seccion3.DatosContacto.telefono);
            this.f_s3_dc_Celular.setValue(metaData.seccion3.DatosContacto.celular);
            this.f_s3_dc_Correo.setValue(metaData.seccion3.DatosContacto.email);
         ...
         if (this.activarPJ) {
            try {
               ...
               console.error(e);
               this.funcionesMtcService.ocultarCargando().mensajeError('El servicio de la SUNAT no se encuentra disponible. Inténtelo mas tarde.');
   ...
   guardarFormulario() {
      ...
      if(this.activarPJ){
         if(this.f_s3_rl_Nombre.value=="" || this.f_s3_rl_ApePaterno.value==""){
            return this.funcionesMtcService.mensajeError('Debe ingresar los datos del representante legal');
         }

         if(this.f_s3_dc_Nombre.value=="" || this.f_s3_dc_ApePaterno.value==""){
            return this.funcionesMtcService.mensajeError('Debe ingresar los datos del contacto');
         }
      }

      let oficinaRepresentante = this.f_s3_rl_Oficina.value;
      let dataGuardar: Formulario001_17_03Request = new Formulario001_17_03Request();

      dataGuardar.id = this.id;
      dataGuardar.codigo = 'F001-17-03';
      ...
      dataGuardar.metaData.seccion3.datosContacto.nombres = this.f_s3_dc_Nombre.value;
      dataGuardar.metaData.seccion3.datosContacto.apellidoPaterno = this.f_s3_dc_ApePaterno.value;
      dataGuardar.metaData.seccion3.datosContacto.apellidoMaterno = this.f_s3_dc_ApeMaterno.value;
      dataGuardar.metaData.seccion3.datosContacto.tipoDocumento.id = this.f_s3_dc_TipoDocumento.value;
      dataGuardar.metaData.seccion3.datosContacto.tipoDocumento.documento = (this.tipoSolicitante === "PJ" ? this.listaTiposDocumentos.filter(item => item.id == this.f_s3_dc_TipoDocumento.value)[0].documento : "");
      dataGuardar.metaData.seccion3.datosContacto.numeroDocumento = this.f_s3_dc_NumeroDocumento.value;
      dataGuardar.metaData.seccion3.datosContacto.telefono = this.f_s3_dc_Telefono.value;
      dataGuardar.metaData.seccion3.datosContacto.celular = this.f_s3_dc_Celular.value;
      dataGuardar.metaData.seccion3.datosContacto.email = this.f_s3_dc_Correo.value;
      ...
      this.visorPdfArchivosService.get(this.uriArchivo)
         ...
         .subscribe(
            (file: Blob) => {
               ...
               modalRef.componentInstance.titleModal = "Vista Previa - Formulario 001/17.03";
