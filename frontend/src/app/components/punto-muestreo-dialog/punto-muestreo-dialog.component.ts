...
@Component({
  selector: 'punto-muestreo-dialog',
  templateUrl: './punto-muestreo-dialog.component.html',
})
export class PuntoMuestreoDialog implements OnInit {
  form: FormGroup;
  selectedUnit: string = '';
  ...
  private patchFormValues(data: FormularioSolicitudDIA): void {
    this.documentos = data?.PuntoMuestreo || [];
  }

  save(form: FormGroup) {
    this.data.PuntoMuestreo = this.documentos;
    this.data.SavePuntoMuestreo = true;
    this.data.FechaRegistroPuntoMuestreo = this.funcionesMtcService.dateNow();
  ...
  ver() {
    if (this.estadoSolicitud !== 'EN PROCESO') { return true; }
    return false;
  }
